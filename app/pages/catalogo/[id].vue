<template>
  <div class="min-h-full bg-gray-50/60 p-3 sm:p-8">
    ...
    <div v-else-if="!veiculo" class="flex flex-col items-center justify-center gap-4 py-32">
      ...
      <NuxtLink to="/catalogo" class="inline-flex items-center gap-2 text-sm font-bold text-amber-600 hover:underline">
    ...
    <!-- Conteúdo -->
    <div v-else>
      <!-- Botão voltar + copiar link -->
      <div class="flex items-center justify-between mb-6">
        <NuxtLink to="/catalogo" class="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-gray-800 group transition-colors">
        ...
        <button
          type="button"
          class="inline-flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-xl border transition-all duration-200"
          :class="linkCopiado
            ? 'bg-green-50 border-green-300 text-green-700'
            : 'bg-white border-gray-200 text-gray-600 hover:border-amber-400 hover:text-amber-700 shadow-sm'"
          @click="copiarLink"
        >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8">
        ...
        <div class="lg:col-span-3 flex flex-col gap-3">
          ...
          <div class="relative rounded-2xl overflow-hidden bg-gradient-to-br from-gray-100 to-gray-200 shadow-md" style="aspect-ratio:16/9">
            <img
              v-if="fotos.length > 0"
              :src="fotos[fotoAtiva]"
              :alt="`${veiculo.marca} ${veiculo.modelo} - foto ${fotoAtiva + 1}`"
            ...
            <template v-if="fotos.length > 1">
              <button
                type="button"
                class="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 hover:bg-black/70 flex items-center justify-center text-white transition-colors backdrop-blur-sm"
                @click="fotoAtiva = (fotoAtiva - 1 + fotos.length) % fotos.length"
              >
              ...
              <button
                type="button"
                class="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 hover:bg-black/70 flex items-center justify-center text-white transition-colors backdrop-blur-sm"
                @click="fotoAtiva = (fotoAtiva + 1) % fotos.length"
              >
              ...
              <!-- Contador -->
              <span class="absolute bottom-3 right-3 text-[11px] font-bold bg-black/60 text-white px-2.5 py-1 rounded-full backdrop-blur-sm">
        ...
        <div class="lg:col-span-2 flex flex-col gap-4">
          ...
          <div v-if="veiculo.chassi || veiculo.renavam" class="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5">
            <h2 class="text-sm font-bold text-gray-700 uppercase tracking-widest mb-4">Dados Técnicos</h2>
            <div class="flex flex-col gap-3">
              <div v-if="veiculo.chassi" class="flex flex-col gap-0.5">
                <span class="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Chassi</span>
                <span class="text-sm font-mono font-bold text-gray-700 break-all">{{ veiculo.chassi }}</span>
              </div>
              <div v-if="veiculo.renavam" class="flex flex-col gap-0.5">
                <span class="text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Renavam</span>
                <span class="text-sm font-mono font-bold text-gray-700">{{ veiculo.renavam }}</span>
              </div>
            </div>
          </div>
...
onMounted(async () => {
  ...
  const { data } = await supabase
    .from('veiculos')
    .select('id,marca,modelo,ano_fabricacao,ano_modelo,tipo,placa,cor,km,combustivel,cambio,cilindrada,chassi,renavam,preco_venda,status,fotos,observacao')
    .eq('id', id)
    .maybeSingle()
